...
Usage: $0 [--system|--user|--all]
  ...
  (default)  Auto-detect: scan /usr/bin, /usr/local/bin, ~/.local/bin
             and remove every ${zacxiom} found. Sudo only for system paths.
...
echo ">> Uninstalling ${zacxiom}"

case "${MODE}" in
    --system)
        for p in "${SYSTEM_PATHS[@]}"; do
            remove_at "${p}/${zacxiom}" yes
        done
        ;;
    --user)
        remove_at "${USER_PATH}/${zacxiom}" no
        ;;
    --all)
        for p in "${SYSTEM_PATHS[@]}"; do
            remove_at "${p}/${zacxiom}" yes
