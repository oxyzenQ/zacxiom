...
Usage: $0 [--system|--user]
  ...
  --system   Install system-wide to /usr/bin/${zacxiom}
             (script invokes sudo for the install step only)
  --user     Install to ~/.local/bin/${zacxiom}  (default, no sudo)
...
echo ">> [1/3] Building ${zacxiom} (release, locked)"
cargo build --release --locked

BINARY="target/release/${zacxiom}"
if [[ ! -f "${BINARY}" ]]; then
    echo "error: build produced no binary at ${BINARY}" >&2
    exit 1
fi

echo ">> [2/3] Installing ${zacxiom} (${MODE})"

case "${MODE}" in
    --system)
        # Invoked WITHOUT sudo; escalate only for the install step.
        sudo install -Dm755 "${BINARY}" "/usr/bin/${zacxiom}"
        echo "   installed: /usr/bin/${zacxiom}"
        ;;
    --user)
        user_bin="${HOME}/.local/bin"
        mkdir -p "${user_bin}"
        install -Dm755 "${BINARY}" "${user_bin}/${zacxiom}"
        echo "   installed: ${user_bin}/${zacxiom}"
        ;;
esac
